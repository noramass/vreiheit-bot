export type ThenHandler<T, R> = (result: T) => PromiseOr<R>;
...
export function isPromise<T = unknown, C = unknown>(
  it: any,
): it is Promise<T> & Thenable<T> & Catchable<C> {
  return isThenable(it) && isCatchable(it);
}
